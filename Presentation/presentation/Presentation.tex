...
\includegraphics[width=1.1cm]{figures/upmc.png}
 \end{textblock}
}
...
\includegraphics[width=2.2cm]{figures/upmc.png}\\
\includegraphics[width=2.5cm]{figures/wizzilab.png}\\
\includegraphics[width=2.5cm]{figures/dash7.JPG}
\column{7cm}
\vspace{1cm}\\
\LARGE{\textbf{\theTitle}}\\
\vspace{0.5cm}
...
\newcommand{\theTitle}{Nouveaux Codages du Canal pour Réseau de Capteurs DASH7}
\newcommand{\thefootnotedef}{Nouveaux Codages du Canal pour Réseau de Capteurs DASH7}
\newcommand{\theAuthors}{shahab SHARIAT BAGHERI}
\newcommand{\theResponsible}{Tuteur de l'entreprise: Yordan TABAKOV\\Tuteur Académique de l'école : Ghaya REKAYA BEN OTHMAN \\ Tuteur Académique de l'UPMC: Julien SARRAZIN\\ Résponsables de Master (TELECOM): Philippe GALLION- Georges Rodriguez\\ Résponsable de Master (UPMC): Muriel DARCES }
\newcommand{\theConferenceAndPlace}{Soutenance de Stage de Master STN \\
Salle A310, TELECOM ParisTech\\ Le 5 Janvier au 5 Juillet\\21/09/2015}
\newcommand{\theDate}{21/09/2015}
...
\section{Une Généralité de Stage}

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsection{DASH7 et Wizzilab}

\begin{frame}{Technologie et DASH7}

\uncover<1->{- Internet des Objets (IoT)$\rightarrow$ 50 milliards d'objets connectés en 2020.}
\uncover<2->{$\rightarrow$ Les Capteurs} 
\uncover<1->{
\begin{center}
\includegraphics[scale=0.25]{figures/iot.png}
\end{center}
}

\uncover<3->{
- \textbf{D}evelopers \textbf{A}lliance for \textbf{S}tandards \textbf{H}armonization
$\rightarrow$ ISO 18000-7 (433MHz): Standard pour technologie de RFID}
\uncover<4->{$\rightarrow$ Protocole pour Réseau de Capteurs sans fil (WSN)}

\uncover<3->{
\begin{center}
\includegraphics[scale=1.1]{figures/dash7.JPG}
\end{center}}

%\vspace{-0.5cm}
%\uncover<1->{\begin{block}{Characteristics of a Good Communication System}
%	\centering
%	\begin{itemize}
%	\item Transmission with higher rates.
%	\item Acceptable Quality of Service (QoS).
%	\item Reliable noise-free communication.
%	\item Assuring mobility \& good connectivity coverage.
%	\end{itemize}}
%\uncover<2->{
%	 $\rightarrow$ The communication between different systems is achieved via \textit{wireless communication mediums} ! 
%	 \end{block}}
%\uncover<3->{\begin{block}{Why we need to employ Interference Alignment (IA) Techniques?} 
%  \centering
%  \begin{itemize}
%   \item Wireless cells are becoming smaller and smaller.
%   \item The density of users is becoming higher and higher in each cell.
%   \item Nodes are receiving some combination of unwanted signals.
%   \item Interference between users (or channels) are increasing.
%   \item Wireless networks have become more and more \textcolor{red}{\textit{Interference Limited}}.
%  \end{itemize}}
%\uncover<4->{
%  $\rightarrow$ To \underline{\textit{manage interference}} and achieve some \underline{\textit{higher rates}} we need to use IA techniques.\end{block}}
\end{frame}


\begin{frame}{Wizzilab}

-En France, Réseau de Capteurs de DASH7, Paris, 75018

\only<1>{
\begin{center}
\includegraphics[scale=0.42]{figures/wizzilab.jpg}
\end{center}}
\only<2>{
\begin{center}
\includegraphics[scale=0.55]{figures/wizziguy.png}
\end{center}}
...
- Les Sensors Developpés par Wizzilab.
\begin{center}
\only<1>{\includegraphics[scale=0.25]{figures/sensor.jpeg}}
\end{center}
- \only<1>{Application}  :\\ 
\only<1>{\includegraphics[scale=0.15]{figures/parking.png}}
\only<1>{\includegraphics[scale=0.15]{figures/parking_lots.jpg}}
\only<2>{\includegraphics[scale=0.55]{figures/network.png}}
\end{frame}


%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

\subsection{Les Objectifs et Outils}
\begin{frame}
\frametitle{Les Objectifs et Outils}
%\vspace{-0.5cm}
\uncover<1->{
\begin{block}{Objectif de Stage}
Développer la Couche Physique de DASH7 $\rightarrow$ Modèle Mathématiques de Système de Communication (DASH7) \uncover<2->{$\rightarrow$ Simulator.}\\
\begin{center}
\includegraphics[scale=0.3]{figures/layer.jpg}
\end{center}
\end{block}
}


\uncover<2->{\includegraphics[scale=0.22]{figures/octave.jpeg}}
\uncover<2->{\includegraphics[scale=0.25]{figures/matlab.jpeg}}
\uncover<3->{\includegraphics[scale=0.25]{figures/linux.jpeg}}
\uncover<3->{\includegraphics[scale=0.2]{figures/git.jpeg}}
\uncover<3->{\includegraphics[scale=0.25]{figures/tricount.png}}
%
%\uncover<1->{
%  \begin{block}{Conventional methods to avoid Interference:}
%  \centering
%  \begin{itemize}
%   \item Frequency Division Multiple Access (FDMA-Using different Sub-Bands)
%   \item Time Division Multiple Access (TDMA-Using different Time-Slots)
%  \end{itemize}}
%  \pause
% $\rightarrow$ \underline{\textit{Avoiding interference}} considered as a solution to overcome the effects of interference.
%\end{block}
%\vspace{-0.1cm}
%\uncover<3->{
%  \begin{block}{Different Approaches of Interference Alignment}
%  \centering
%  \begin{itemize}
%   \item Linear IA when time, space and frequency diversity is available [S. A. Jafer 2011].
%   \item Non-linear IA for static single antenna channels [S. A Motahari \textit{et al.} 2009].
%  \end{itemize}
%\end{block}}
%\vspace{-0.1cm}
%\uncover<4->{
...
%  \centering
%  \begin{itemize}
%   \item 2-User channel is introduced many years ago by [C. E. Shannon 1962].
%   \item The 2-User Interference channel defined by [R. Ahlswede 1974].\\
%   \underline{Additive Gaussian noise} is present at each receiver.
%  \end{itemize}
%  \pause
%   $\rightarrow$ Open problem is to find all the characteristics of this model (Capacity, DoF,...) before passing to a general $K-$User channel !
%\end{block}}
\end{frame}
...
%%%%%%%%%%%%%%%%%%%%%%%
\section{Les Développements sur DASH7}

\subsection{Modulation et Définition des Canaux adjacents}
